...
step3_get_analysis_z_scores_experimental <- function(
  ...
  z_score_calc <- function(
    ...
    internal_stats_function <- function(
      internal_probe_ID,
      internal_hyper_hypo
    ){
      ...
      ## Remove the NA methylation probes:
      T_P_probe <- T_P_probe[
        !is.na(T_P_probe)
      ]

      ## Get the probes with 1 values:
      T_P_probe_1 <- T_P_probe[(T_P_probe==1)]

      ## Get the probes with 0 values:
      T_P_probe_0 <- T_P_probe[(T_P_probe==0)]

      ## Get info for the samples that are hypo/hypermethylated:
      if(internal_hyper_hypo=='hyper'){

        group_of_interest_count <- length(T_P_probe_0)
        other_group_count <- length(T_P_probe_1)

        group_methylation <- metData[
          internal_probe_ID,
          names(T_P_probe_0)
        ]

        group_expression <- expData[
          gene_ENSG,
          names(T_P_probe_0)
        ]

      } else if(internal_hyper_hypo=='hypo'){

        group_of_interest_count <- length(T_P_probe_1)
        other_group_count <- length(T_P_probe_0)
      ...
      if((group_of_interest_count>minExp_value) & (other_group_count>0)){

        ## Get the expression values from the samples
        ## depending on their inclusion in the
        P_gene_1 <- expData[
          gene_ENSG,
          names(T_P_probe_1)
        ]
        ...
        ## Get perform the expression z-score calculation:
        z_score_internal <- (
          (mean(P_gene_0, na.rm=T) - mean(P_gene_1, na.rm=T))/
            (sd(P_gene_1, na.rm=T))
        )

        ## Perform linear regression of the methylation values for the
        ## group of interest onto the expression for the group of interest
        expression_methylation_regression <- lm(
          group_methylation~group_expression
        )
        ...
        ## Return info for Zscore calculation and Regression
        return(
          c(
            z_score_internal,
            p_value,
            beta_value_internal,
            group_of_interest_count,
            other_group_count
          )
        )

      } else if((group_of_interest_count>minExp_value) & (other_group_count==0)){
        ...
        ## Return info for regression with
        ## NA value for the Z score calculation
        return(
          c(
            NA,
            p_value,
        ...
        ## Return only values for the sample counts for that individual
        ## probe/gene:
        return(
          c(
            NA,
      ...
      ## clear memory:
      if(
        exists('expression_methylation_regression')
      ){

        rm(expression_methylation_regression)
      }

      rm(group_methylation)
      rm(group_expression)
    }
    ...
    if(hyper_hypo=='hyper'){
      ...
      TESTSR <- cbind(
        ...
        t(
          sapply(
            TESTSR$probeID,
            internal_stats_function,
            internal_hyper_hypo='hyper'
          )
        )
      )

      ## Change the column names to match the created files:
      colnames(TESTSR) <- c(
        'geneID',
        'probeID',
        'Z.real',
