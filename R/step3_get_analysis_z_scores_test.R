#' step3_get_analysis_z_scores_test
#'
...
step3_get_analysis_z_scores_test <- function(
  TENET_directory,
  hypermeth_analysis,
  hypometh_analysis,
  usecaseonly=TRUE,
  ...
  z_score_calc <- function(
    ...
    internal_probe_z_function <- function(
      ...
      if(length(T_P_probe_1)>min_experimental_count_value & length(T_P_probe_0)>0){
  ...
  if(hypermeth_analysis==TRUE){
    ...
    parallel::mclapply(
      ...
      hyper_hypo= 'hyper',
      min_experimental_count_value= min_experimental_count,
      mc.cores= core_count
    )
